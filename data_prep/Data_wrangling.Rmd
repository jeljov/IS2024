...
```
Note that for `Milk` and `Size` we, in fact, have 5 and 4 distinct values, respectively, as one of the values is an indicator of the missing value(s). 
...
### Categorical or ordinal variables with a small number of missing values

If there are only a couple of missing values in a factor variable, they are replaced by the 'majority class', that is, the most dominant value of that variable. When doing so, it is recommended to identify the dominant value among the subset of observations that are similar to the one with the missing value. For example, in our case, we can identify the dominant value for `Milk` among the beverages that the one with the missing value belongs to, since 'Milk' and 'Beverage' proved to be significantly associated:

```{r warning=FALSE}
table(sb_drinks$Beverage, sb_drinks$Milk) |> chisq.test()
```

```{r}
milk_missing_beverage <- sb_drinks$Beverage[is.na(sb_drinks$Milk)]
milk_missing_beverage
```

```{r}
sb_drinks$Milk[sb_drinks$Beverage == milk_missing_beverage] |> table()
```
...
Next, we need to handle the missing values in the `Size` variable. Since almost any drink in Starbucks can be of any size, beverage category or group will not be much informative regarding the drink size (can be verified with the Chi-square test - using the `chisq.test()` function), we will use the majority class on the overall data set to fill out the missing values
```{r}
...
In case a numeric variable has a few missing values, we typically replace the missing values with the average value of the variable on a subset of observations that are similar - in "important ways" - to the observation(s) with the missing value. 

For example, in case of the missing value of the `Total_Fat_g` variable, we can find an instance or a couple of them that are most similar in `Trans_Fat_g`, `Saturated_Fat_g`, and `Calories` values. This can be done, for example, using the kNN algorithm. 
...
```{r}
for(i in 1:2) {
  avg_caffeine <- median(sb_drinks$Caffeine_mg[sb_drinks$Beverage_category == caffeine_NA_bev_cat[i] &
                                              sb_drinks$Size == caffeine_NA_size[i]], na.rm = T)
  sb_drinks$Caffeine_mg[sb_drinks$Beverage_category == caffeine_NA_bev_cat[i] &
                          sb_drinks$Size == caffeine_NA_size[i] &
...
For continuous variables, we use histogram or probability density function to inspect the relation between an input variable and the outcome. In such plots, we examine the presence or extent of difference in the distribution of the input variable for the different values of the outcome variable. For example, we can examine how relevant the quantity of sodium is for distinguishing between less and more healthy drink choices:  
```{r}
ggplot(sb_drinks,
       aes(x = Sodium_mg, fill = HealthierChoice)) +
  geom_density(alpha=0.45) +
  theme_minimal()
```
We can observe a clear difference in the distributions: healthier choices tend to have lower quantity of sodium. So, the `Sodium_mg` variable is expected to be a relevant predictor.  

Let's take one more more - for example, `Dietary_Fibre_g`
```{r}
ggplot(sb_drinks,
       aes(x = Dietary_Fibre_g, fill = HealthierChoice)) +
  geom_density(alpha=0.65) +
  theme_minimal()
```
Healthier choices tend to have higher values of dietary fiber and very different distribution than less healthy choices, suggesting that `Dietary_Fibre_g` is also expected to be a good predictor of the outcome variable.

If it's easier for you, you may also use boxplots:
```{r}
ggplot(sb_drinks,
       aes(x = HealthierChoice, y = Sodium_mg, fill = HealthierChoice)) +
  geom_boxplot() +
  theme_minimal()
```
...
factor_var_names = colnames(sb_drinks)[c(3,4,17,18)]
...
```{r}
summary(aov(sb_drinks$Vitamin_A_DV ~ sb_drinks$HealthierChoice))
```
