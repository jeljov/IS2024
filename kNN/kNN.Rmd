...
So, let's check if the input variables have outliers 
```{r}
...
```{r}
set.seed(2024)
training_indices = createDataPartition(mobiles_norm$top_price, p = 0.8, list = FALSE)
...
```{r}
set.seed(2024)
rand_k = sample(seq(1,15,2), 1) ## reminder: we consider only odd values for k, to avoid ties
